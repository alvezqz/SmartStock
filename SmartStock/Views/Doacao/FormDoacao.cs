...
namespace SmartStock.Views.Doacao
{
    public partial class FormDoacao : Form
    {
        ...
        private void CarregarDoacao()
        {
            ...
            if (_idDoacao != null)
            {
                ...
                if (dt != null)
                {
                    ...
                    _idProduto = int.Parse(comboProduto.SelectedValue.ToString());
                    ...
                    quantDoada = int.Parse(numQuantidade.Value.ToString());
        ...
        {
            if (int.Parse(comboProduto.SelectedValue.ToString()) >= 0)
            {
        ...
        private bool ValidarForm()
        {
            ...
            if (numQuantidade.Value <= 0 || !int.TryParse(numQuantidade.Value.ToString(), out int quant))
            ...
            try
            {
                if (ValidarForm())
                    if (_idProduto > 0)
                    {
                        ...
                        {
                            string query = "UPDATE Doacao SET nomeInstituicao = @nomeInstituicao," +
                                " quantidade = @quantidade, dataDoacao = @dataDoacao, status = @status, idProduto = @idProduto " +
                            ...
                            if (resultado)
                            {
                                ...
                                {
                                    new MySql.Data.MySqlClient.MySqlParameter("@idProduto", _idProduto),
                                    new MySql.Data.MySqlClient.MySqlParameter("@quantidadeAtual", quantMax + quantDoada - numQuantidade.Value)
                                });
